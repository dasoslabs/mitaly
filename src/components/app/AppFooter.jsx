    ...
    <footer className="max-w-pc m-auto py-12">
          ...
          <ul className="flex flex-wrap">
            {
              data.map(({ title, text }, idx) => (
                <li key={title} className="mb-3">
                  ...
                  <span>{text}</span>
                  { (idx + 1) % 3 !== 0 && <span className="px-2 text-[#D9D9D9]">|</span> }
                </li>
              ))
            }
          </ul>
      ...
      <div className="flex justify-between items-center">
        <ul className="flex space-x-4">
          {
            ["개인정보처리방침", "서비스 이용약관", "영상정보처리방침"].map((text) => (
              <li key={text}>
                <Link href="#">{text}</Link>
              </li>
            ))
        ...
        <p className="font-bold">ⓒ copyright 2023 Truefarm co., itd all rights reserved.</p>
      </div>
    </footer>
  )
}