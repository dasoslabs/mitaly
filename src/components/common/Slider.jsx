...
export default function Slider({ children }) {
  ...
  return (
    <div className="relative">
      ...
      <PrevButton onClick={onPrevButtonClick} disabled={prevBtnDisabled} className="absolute -left-16 top-1/2 transform -translate-y-1/2" />
      <NextButton onClick={onNextButtonClick} disabled={nextBtnDisabled} className="absolute -right-16 top-1/2 transform -translate-y-1/2" />
    </div>
  )
}
...
export function SliderCard({ children, className, ...props }) {
  return (
    <div className={`min-w-0 flex-grow-0 flex-shrink-0 basis-1/4 ${className}`} {...props}>
      {children}
    </div>
  )
}